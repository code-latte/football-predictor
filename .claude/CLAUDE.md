...
- **FluentMigrator** for all DB schema migrations, applied automatically on startup. EF Core is the ORM (querying/persistence) but is not used for schema management. → `docs/adr/0006-reinstate-fluentmigrator.md`
...
`{Service}.Tests.Infrastructure` references `{Service}.Migrations` and runs the FluentMigrator runner against a containerised database to build the schema from scratch before integration tests run.
...
- Location: `{Service}.Migrations` project under `src/` inside each service — FluentMigrator `Migration` subclasses only.
- Auto-applied on startup: `DatabaseInitializer.EnsureCreatedAsync` creates the database if absent, then `runner.MigrateUp()` applies pending migrations.
- Migration name format: attribute `[Migration(YYYYMMDDHHmm)]`, class name `{PascalCaseDescription}` (e.g. `[Migration(202401150930)]` on class `CreatePredictionsTable`).
- The `DbContext` lives in `{Service}.Infrastructure` — not in `{Service}.Migrations`.
...
| ADR: FluentMigrator | `docs/adr/0003-use-fluentmigrator.md` |
| ADR: Sentry frontend monitoring | `docs/adr/0004-use-sentry-for-frontend-monitoring.md` |
| ADR: FluentMigrator (reinstated) | `docs/adr/0006-reinstate-fluentmigrator.md` |
