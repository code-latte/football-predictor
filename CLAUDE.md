...
- **FluentMigrator** for all DB migrations, applied automatically on startup. → `docs/adr/0003-use-fluentmigrator.md`
...
|---|---|---|
| Unit | NUnit (C#), Jest + RTL (TS) | ≥ 70% domain layer coverage |
| Integration | NUnit + Testcontainers (PostgreSQL + RabbitMQ) | At least one integration test per service |
...
- Location: `src/Infrastructure/Migrations/` inside each service.
- Auto-applied on startup via `IMigrationRunner.MigrateUp()`.
- Version format: `YYYYMMDDHHmm` (12-digit timestamp).
- Class naming: `M{version}_{PascalCaseDescription}` (e.g. `M202401150930_CreatePredictionsTable`).

**Hard rules:**
- Never modify an already-applied migration — add a new one.
- Always implement `Down()`.
- One logical change per migration.
- No business logic in migrations — schema only.
